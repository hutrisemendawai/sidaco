<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Filter Section --}}
            <div x-data="{ open: {{ $request->anyFilled(['year', 'month', 'country', 'province', 'species']) ? 'true' : 'false' }} }" class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
...
<div>
        ...
        @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div class="bg-white p-6 rounded-lg shadow-sm flex items-center space-x-4">
                    ...
                    <div>
                        <div class="text-gray-500 text-sm">Unique Country</div>
                        <div x-data="{ count: 0 }" x-init="() => { let final = {{ $uniqueCountry }}; let start = 0; let duration = 1500; let step = Math.ceil(final / (duration / 16)); setInterval(() => { start += step; if (start >= final) { count = final; return; } count = start; }, 16); }" class="text-2xl font-bold text-gray-900" x-text="count"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {{-- Chart Cards --}}
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg"><div class="p-6 text-gray-900"><h3 class="text-lg font-medium text-gray-900 mb-4">Total Weight Caught Per Year (kg)</h3><div class="h-64"><canvas id="yearlyCatchChart"></canvas></div></div></div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg"><div class="p-6 text-gray-900"><h3 class="text-lg font-medium text-gray-900 mb-4">Data Entries by Country</h3><div class="h-64"><canvas id="countryChart"></canvas></div></div></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const countryCtx = document.getElementById('countryChart').getContext('2d'); new Chart(countryCtx, { type: 'bar', data: { labels: @json($countryLabels), datasets: [{ label: 'Number of Data Entries', data: @json($countryCounts), backgroundColor: 'rgba(22, 163, 74, 0.5)', borderColor: 'rgba(22, 163, 74, 1)', borderWidth: 1 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } }, plugins: { legend: { display: false } } } });
