...
class FishingDataImport implements ToModel, WithHeadingRow
{
    public function model(array $row)
    {
        ...
        return new SidatData([
            ...
            'country'               => $row['country'] ?? 'unkown',
            'province'              => $row['province'] ?? 'unkown',
            'regency'               => $row['regency'] ?? 'unkown',
            'district'              => $row['district'] ?? 'unkown',
            'river'                 => $row['river'] ?? 'unkown',
            'stage'                 => $row['stage'] ?? 'unkown',
            'fisher_name'           => $row['fisher_name'] ?? 'unkown',
            'number_of_fisher'      => $row['number_of_fisher'] ?? 0,
            'type_of_fishing_gear'  => $row['type_of_fishing_gear'] ?? 'unkown',
            'number_of_fishing_gear' => $row['number_of_fishing_gear'] ?? 0,
            'species_name'          => $row['species_name'] ?? 'unkown',
